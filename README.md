...
Каждые N ходов (по умолчанию — 10) ИИ обновляет `summary.txt` — сжатую хронику всей истории. Файл учитывает карточки историй и предыдущее summary, поэтому важные факты сохраняются даже когда свежая история обрезается из-за лимита контекста.
...
| `max_tokens` | `400` | Максимум токенов в ответе |
| `context_size` | `24576` | Размер контекстного окна модели |
